...
const sections: Array<{ id: HelpSectionId; title: string; content: ReactNode }> = [
  ...
  { id: 'day', title: '하루가 진행되는 방식', content: <p>거래일은 <strong>개장 전 → 장중 → 장 마감</strong> 순서입니다. 먼저 장을 시작해 실제 시가를 확인하고, 장중에는 그 시가로 주문한 뒤 장을 마감합니다. 거래일은 반드시 장을 마감한 뒤에만 다음 날짜로 이동할 수 있습니다.</p> },
  { id: 'orders', title: '주문과 시가 체결', content: <p><strong>장 시작 후 공개된 당일 실제 시가</strong>가 그날의 주문 체결가입니다. 장 마감 전까지 매수·매도 주문은 같은 시가로 즉시 체결되며, 주문 입력 중 수량·주식 금액·수수료·총 필요금액 또는 예상 정산액을 미리 확인할 수 있습니다.</p> },
...
const replaySteps = [
  ...
  ['4. 장 마감', '장을 마감하면 고가·저가·종가를 포함한 전체 OHLC가 공개되고 다음 날짜로 이동할 수 있습니다. 중요 이벤트에서는 자동진행이 멈춥니다.'],
] as const
