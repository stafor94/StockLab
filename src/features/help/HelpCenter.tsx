...
const sections: Array<{ id: HelpSectionId; title: string; content: ReactNode }> = [
  ...
  { id: 'day', title: '하루가 진행되는 방식', content: <p>거래일은 <strong>개장 전 → 장중 → 장 마감</strong> 순서입니다. 장을 시작하면 실제 시가가 공개되어 시가로 거래할 수 있고, 장을 마감하면 실제 종가와 전체 OHLC가 공개되어 종가로 한 번 더 거래할 수 있습니다. 거래일은 반드시 장을 마감한 뒤에만 다음 날짜로 이동할 수 있습니다.</p> },
  { id: 'orders', title: '시가·종가 주문', content: <p><strong>장 시작 후에는 당일 실제 시가</strong>로, <strong>장 마감 후에는 당일 실제 종가</strong>로 매수·매도 주문이 즉시 체결됩니다. 두 가격은 모두 실제 비조정 가격을 사용하며, 주문 입력 중 수량·주식 금액·수수료·총 필요금액 또는 예상 정산액을 미리 확인할 수 있습니다.</p> },
  { id: 'prices', title: '가격·차트 공개 범위', content: <p>개장 전에는 당일 OHLC를 숨기고, 장중에는 시가만 공개합니다. 장 마감 후에만 당일 전체 OHLC와 완성된 차트를 공개하며, 이때 공개된 종가로 주문할 수 있습니다.</p> },
  ...
  { id: 'autoplay', title: '자동진행 중단', content: <p>자동진행은 거래일에 장 시작과 장 마감을 자동으로 진행하지만 임의로 매매하지 않습니다. 중요 뉴스, 기업 이벤트, 대출 자동출금 실패, 게임오버, 필수 데이터 오류에서는 자동진행이 멈춥니다.</p> },
]

const replaySteps = [
  ['1. 개장 전', '당일 가격은 보이지 않습니다. 공개된 과거 정보만 보고 오늘 투자할 종목을 검토하세요.'],
  ['2. 장 시작', '장을 시작하면 당일 실제 시가가 공개됩니다. 장중에는 이 시가로 매수·매도할 수 있고 고가·저가·종가는 아직 보이지 않습니다.'],
  ['3. 시가 주문', '시장 탭에서 종목을 선택하고 수량이나 금액을 입력하세요. 수수료를 포함한 총 필요금액을 확인한 뒤 같은 시가로 즉시 체결할 수 있습니다.'],
  ['4. 장 마감·종가 주문', '장을 마감하면 고가·저가·종가를 포함한 전체 OHLC가 공개됩니다. 이때 공개된 실제 종가로 매수·매도한 뒤 다음 날짜로 이동할 수 있습니다.'],
] as const
