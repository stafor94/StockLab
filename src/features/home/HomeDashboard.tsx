...
export function HomeDashboard({ onOpenMarket }: HomeDashboardProps) {
  ...
  const timelineReady = Boolean(calendars && rateState.status === 'ready' && rateState.baseRate !== null)
  const advanceDate = (step: GameDateStep) => {
    if (!calendars || rateState.status !== 'ready') return
    ...
    const result = game.advanceToDate(nextDate, { baseRates: rateState.series, bankBusinessDates: calendars.KR.tradingDates })
    if (!result.ok) {
      setTimelineMessage(result.message)
      return
    }
    const prefix = cancelledOrders > 0 ? `미체결 주문 ${cancelledOrders}건 취소 · ` : ''
    setTimelineMessage(result.message ? `${prefix}${result.message}` : cancelledOrders > 0 ? `${prefix}${nextDate}로 이동했습니다.` : null)
  }
  ...
  return (
    <main className="dashboard">
      ...
      <section className="content-grid">
        ...
        <article className="panel news-panel"><div className="panel-heading"><div><p className="section-label">DAILY NEWS</p><h2>오늘의 뉴스</h2></div><span className="count-badge">0</span></div><div className="empty-state"><strong>2018년 시장 데이터 준비 중</strong><p>뉴스는 해당 게임 날짜가 된 뒤에만 공개됩니다.</p></div></article>
      </section>

      <section className="panel timeline-panel">
        <div><p className="section-label">TIME CONTROL</p><h2>시간 진행</h2><p className="timeline-note" aria-live="polite">{timelineMessage ?? (timelineReady ? '날짜 진행 시 결제대금 입금 → 대출 이자/재출금 순서로 자동 처리됩니다.' : rateState.status === 'unavailable' ? '한국은행 기준금리 데이터가 없어 시간을 진행할 수 없습니다.' : '시장 캘린더와 기준금리를 불러오는 중입니다.')}</p></div>
