...
export function LoanScreen() {
  ...
  const [visibleHistoryCount, setVisibleHistoryCount] = useState(LOAN_HISTORY_PAGE_SIZE)
  ...
  const visibleHistory = useMemo(() => game.loan.history.filter((event) => event.type !== 'interest_due').slice().reverse(), [game.loan.history])
  const visibleHistoryItems = visibleHistory.slice(0, visibleHistoryCount)
  const hasMoreHistory = visibleHistoryCount < visibleHistory.length
  const marketOpen = game.marketSessions.KR.phase === 'opened' || game.marketSessions.US.phase === 'opened'
  const repaymentDisabled = game.loan.status === 'paid' || game.loan.status === 'overdue' || marketOpen || !Number.isFinite(amount) || amount <= 0
    ...
    <div className="loan-screen">
      ...
      <section className="loan-history">
        ...
        {visibleHistory.length === 0
          ? <p>아직 납부 또는 상환 내역이 없습니다.</p>
          : <>
              <div className="loan-history-list">
                {visibleHistoryItems.map((event) => <div key={event.id}><span>{event.date} · {eventLabels[event.type]}</span><strong>{formatMoney(event.amount, 'KRW')}</strong><small>{event.note}</small></div>)}
              </div>
              {hasMoreHistory && <button className="loan-history-more" type="button" onClick={() => setVisibleHistoryCount((count) => count + LOAN_HISTORY_PAGE_SIZE)}>더보기</button>}
            </>}
      </section>
    </div>
  )
