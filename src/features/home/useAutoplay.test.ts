...
describe('useAutoplay', () => {
  ...
  it('supports 30x as UI timing without overlapping ticks', async () => {
    vi.useFakeTimers()
    const tick = vi.fn().mockReturnValue(false)
    const { result } = renderHook(() => useAutoplay(tick, false))
    act(() => { result.current.setSpeed(30); result.current.start() })
  ...
  it('stops when an in-flight tick returns false even if blocking state changed during the tick', async () => {
    vi.useFakeTimers()
    let resolveTick: ((value: boolean) => void) | null = null
    const tick = vi.fn(() => new Promise<boolean>((resolve) => { resolveTick = resolve }))
    const { result, rerender } = renderHook(({ blocked }) => useAutoplay(tick, blocked), {
