...
test('keeps the core game actions and five-screen navigation available', async ({ page }) => {
  ...
  await expect(page.getByText('v0.15.2')).toBeVisible()
...
test('responsive layout avoids overflow and keeps touch targets usable', async ({ page }, testInfo) => {
  ...
  expect(headlineFontSize).toBeLessThanOrEqual(36)
  expect(netAssetsBox?.x ?? 0).toBeGreaterThan((headlineBox?.x ?? 0) + (headlineBox?.width ?? 0) - 1)
  expect((netAssetsBox?.x ?? 0) + (netAssetsBox?.width ?? 0)).toBeLessThanOrEqual((viewport?.width ?? 0) + 1)
