...
export function LoanScreen() {
  ...
  return (
    <div className="loan-screen">
      ...
      {game.loan.status === 'overdue' && <section className="loan-overdue-alert" role="alert"><strong>대출이자가 미납되었습니다.</strong><span>미납액 ₩{krw.format(totalPastDue)} · 연속 {game.loan.consecutiveMissedMonths}개월</span><p>원화 현금이 전액 준비된 다음 WS은행 영업일에 자동 재출금합니다. 미결제 매도대금과 달러는 사용할 수 없습니다.</p></section>}
      <section className="loan-summary-grid"><article><span>대출 원금</span><strong>₩{krw.format(game.loan.principal)}</strong></article><article><span>현재 약정금리</span><strong>{annualRate === null ? '—' : `${rate.format(annualRate)}%`}</strong></article><article><span>다음 이자일</span><strong>{game.loan.status === 'paid' ? '—' : nextPaymentDate ?? '확인 불가'}</strong></article><article><span>미청구 이자</span><strong>₩{krw.format(accruedInterest)}</strong></article></section>
      <section className="loan-grid">
        <article className="loan-repayment-card"><h3>원금 중도상환</h3><div className="loan-cash-line"><span>상환 가능한 원화 현금</span><strong>₩{krw.format(game.krwCash)}</strong></div><label className="loan-input"><span>상환할 원금</span><div><b>₩</b><input aria-label="대출 원금 상환액" inputMode="numeric" value={amountText} onChange={(event) => { setAmountText(event.target.value); setMessage(null) }} placeholder="1,000,000" /></div></label><div className="loan-quick-actions"><button type="button" onClick={() => setAmountText(String(WS_PRINCIPAL_REPAYMENT_UNIT))}>100만원</button><button type="button" onClick={() => setAmountText(String(WS_PRINCIPAL_REPAYMENT_UNIT * 3))}>300만원</button><button type="button" onClick={() => setAmountText(String(game.loan.principal))}>전액</button></div>{amount >= game.loan.principal && game.loan.principal > 0 && <div className="loan-payoff-preview">전액상환 시 미청구 이자 약 ₩{krw.format(accruedInterest)}도 함께 정산됩니다.</div>}<button className="primary-button loan-submit" type="button" disabled={repaymentDisabled} onClick={executeRepayment}>원금 상환</button>{game.loan.status === 'overdue' && <p className="loan-message warning">연체 중에는 원금 중도상환보다 미납 이자 자동출금이 우선됩니다.</p>}{message && <p className="loan-message" aria-live="polite">{message}</p>}</article>
      ...
      <section className="loan-history"><h3>대출 내역</h3>{game.loan.history.length === 0 ? <p>아직 이자 청구 또는 상환 내역이 없습니다.</p> : <div className="loan-history-list">{game.loan.history.slice().reverse().slice(0, 30).map((event) => <div key={event.id}><span>{event.date} · {eventLabels[event.type]}</span><strong>₩{krw.format(event.amount)}</strong><small>{event.note}</small></div>)}</div>}</section>
    </div>
  )
}
