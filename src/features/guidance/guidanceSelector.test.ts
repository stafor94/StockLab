...
describe('selectGuidance', () => {
  ...
  it('moves the primary guidance through the session phases', () => {
    ...
    const visited = { ...initial, guidance: { ...initial.guidance, experienced: ['market-visited'] } } satisfies typeof initial
  ...
  it('supplies textual reasons for attention badges', () => {
    ...
    expect(model.navigation.자산).toMatchObject({ attentionCount: 2, attentionReason: expect.stringContaining('결제 실패') })
  })
})
