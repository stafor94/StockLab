...
- The game has one chronological game clock, while KRX and U.S. markets maintain independent trading dates and `preopen` → `opened` → `closed` session state. Market events advance in actual timestamp order; one market closing or being on holiday must not force the other market to the same state.
- During a market's `preopen`, that market's current trading-date open/high/low/close are all hidden and valuation uses its latest completed close.
- During a market's `opened`, only that market's current trading-date actual unadjusted open may be exposed; high, low, and close remain hidden. Immediate orders may execute only for that open market and only at that open price.
