...
function maxAffordableQuantity(budget: number, openPrice: number, order: MarketOrder): number {
...
export function executeMarketOpenOrders(source: TradingAccountState, context: MarketOpenExecutionContext): { state: TradingAccountState; results: OrderExecutionResult[] } {
  ...
  for (const order of orders) {
    if (order.tradeDate !== context.date) { results.push({ orderId: order.id, status: 'cancelled', reason: 'wrong-trade-date' }); continue }
    const openPrice = context.openPrices[order.assetId]
    if (!openPrice || !Number.isFinite(openPrice) || openPrice <= 0) { results.push({ orderId: order.id, status: 'cancelled', reason: 'missing-open-price' }); continue }

    if (order.kind === 'buy-amount' || order.kind === 'buy-quantity') {
      const availableCash = cashForCurrency(state, order.currency)
      const quantity = order.kind === 'buy-amount' ? maxAffordableQuantity(Math.min(order.requestedAmount ?? 0, availableCash), openPrice, order) : order.requestedQuantity ?? 0
      if (quantity <= 0 || !Number.isInteger(quantity)) { results.push({ orderId: order.id, status: 'cancelled', reason: 'invalid-order' }); continue }
      const cost = calculateBuyCashRequired(quantity, openPrice, order.market, order.currency)
      if (cost.total > availableCash) { results.push({ orderId: order.id, status: 'cancelled', reason: 'insufficient-cash' }); continue }
    ...
    if (!position || quantity <= 0 || quantity > position.quantity) { results.push({ orderId: order.id, status: 'cancelled', reason: 'insufficient-position' }); continue }
    const settlementDate = context.settlementDates[order.assetId]
    if (!settlementDate) { results.push({ orderId: order.id, status: 'cancelled', reason: 'missing-settlement-date' }); continue }
