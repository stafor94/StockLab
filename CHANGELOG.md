...
### Added
- 첫 게임의 시장 방문, 종목 상세, 주문 또는 건너뛰기 선택, 장 시작·마감, 다음 게임일 이동을 추적하는 접이식 추천 체크리스트를 추가했습니다.
- 중요 뉴스·기업 이벤트·결제 실패 알림 배지와 현재 단계 대표 행동 강조를 단일 안내 selector에서 계산하도록 추가했습니다.

### Changed
- 주문이 없는 첫 장 시작에서만 짧은 확인을 표시하며, 모션 감소 환경에서는 안내 맥동 대신 정적 테두리를 사용합니다.
- 안내 경험과 체크리스트 상태를 보존하도록 저장 스키마를 v10으로 올리고 기존 저장 데이터 마이그레이션을 추가했습니다.
